# adapted from: https://github.com/babylm/evaluation-pipeline/blob/17b3806376fa90c24653efe0c6c571adfb6a72c7/finetune_classification.py
...
from languini.projects.gpt.finetuning.classification_model import HFClassificationModel, HFClassificationModelConfig
from languini.projects.gpt.finetuning.tokenizer import HFTokenizer
...
assert torch.cuda.device_count() <= 1, "Only implemented for single GPU"
...
class DataTrainingArguments:
    ...
    save_model: bool = field(default=False, metadata={"help": "Whether to save the model after training."})
...
class ModelArguments:
    ...
    """
    L_train: str = field(
        metadata={"help": "Language to train in (L1/L2)."}
    )
    wandb_run: str = field(
        metadata={"help": "Wandb run to load pretrained model and config from"}
    )
    ...
    freeze_embeddings: bool = field(
        default=False,
        metadata={"help": "Whether to freeze the embedding parameters of the base model."}
    )
    cache_dir: Optional[str] = field(
        default=None,
        metadata={"help": "Where do you want to store the pretrained models downloaded from huggingface.co"},
    )
    clf_head_dropout_rate: float = field(
        default=0.1,
        metadata={"help": "Dropout rate for the classification head."}
    )
...
def main():
    ...
    config = HFClassificationModelConfig(
        wandb_run=model_args.wandb_run,
        num_labels=num_labels,
        device=training_args.device,
        clf_head_dropout_rate=model_args.clf_head_dropout_rate,
        L_train=model_args.L_train,
    )
    tokenizer = HFTokenizer(config)
    model = HFClassificationModel(config=config)

    # compile base model if possible (the entire model cannot be compiled because of issues with HF)
    if torch.cuda.is_available():
        cuda_capability = float(torch.cuda.get_device_capability()[0])
        if cuda_capability >= 7.0:
            model.languini_base_model = torch.compile(model.languini_base_model)
    
    if model_args.freeze_model:
        # Freeze all parameters (including embeddings) except the classifier head
        assert not model_args.freeze_embeddings
    ...
    if model_args.freeze_embeddings:
        ...
        assert not model_args.freeze_model
        model.languini_base_model.input_embedding.weight.requires_grad = False
        model.languini_base_model.position_embedding.weight.requires_grad = False
        # ignore output embeddings, they are not used in classification
    ...
    assert data_args.overwrite_cache, "probably unintended"
    ...
    # Run evaluations (and predictions) in both languages (L1 and L2)
    for L_predict in ["L1", "L2"]:
        if model.config.frac_duplicated == 0 and L_predict == "L2":
            # Cannot predict in other language if models wasn't trained under duplication
            continue

        model.set_prediction_language(L_predict)
        ...
        if training_args.do_eval:
            ...
            for eval_dataset, task in zip(eval_datasets, tasks):
                ...
                trainer.log_metrics(f"eval_{L_predict}", metrics)
                trainer.save_metrics(f"eval_{L_predict}", combined if task is not None and "mnli" in task else metrics)

        if training_args.do_predict:
            logger.info("*** Predict ***")

            # We do not use MNLI test data.
            assert data_args.task_name != "mnli"
            ...
            output_predict_file = os.path.join(training_args.output_dir, f"predict_results_{L_predict}.txt")
    ...
    kwargs = {"finetuned_from": model_args.wandb_run, "tasks": "text-classification"}
    if data_args.task_name is not None:
        kwargs["language"] = "en"
        kwargs["dataset_tags"] = "glue"
        kwargs["dataset_args"] = data_args.task_name
        kwargs["dataset"] = f"GLUE {data_args.task_name.upper()}"
        kwargs["L_train"] = model_args.L_train
